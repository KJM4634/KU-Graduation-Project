...
**라벨링**: 이 282장은 Roboflow로 수동 bbox 라벨링을 진행한다 (전체범위 원칙 동일 적용,
단 화면 밖으로 나간 부분은 추정해서 그리지 않음). 업로드 편의를 위해 영상별 하위 폴더
없이 파일명 앞에 영상명을 붙인 평탄 버전을 `model/data/own_capture/subsampled_frames_flat/`
에 별도로 준비해뒀다. 계정 생성부터 클래스 설정, 라벨링 원칙, Roboflow export를
`model/data/own_capture/labeled/{images,labels}/`로 가져오는 절차까지는
**[docs/ROBOFLOW_GUIDE.md](ROBOFLOW_GUIDE.md)** 참고.
...
**규모 판단**: 서브샘플링 후 282장은 목표치(500~1,000장)에는 못 미치지만, **단일 카메라
각도·단일 장소(침대)·8개 영상**에서만 나온 데이터라는 근본적 한계를 감안하면 무리하게
프레임 수를 늘리는 것보다 중복 없는 282장이 라벨링 관점에서 더 실용적이다. 다만 각도/거리/
조명 다양성이 전혀 없다는 한계는 여전하다. 이 282장은 **occlusion이 연속적으로 진행되는
과정을 촘촘하게 담은 보조 데이터**로는 매우 유용하지만, SHOOTING_GUIDE.md가 원래 요구한
"각도/거리/조명 다양성"을 대체할 수는 없다. **권장**: 팀원 C·D에게 최소 2~3개 각도
...
model/data/
  ...
  own_capture/            # 자체 촬영 (gitignore 처리)
    ...
    subsampled_frames_flat/  # 위와 동일한 282장, Roboflow 업로드용 평탄 폴더
