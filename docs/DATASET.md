...
2. `model/scripts/process_own_capture_video.py` — YOLO11로 검출한 person bbox의 종횡비
   ...
   - "누워있는(가려짐 진행 중)" 프레임 → `model/data/own_capture/frames_lying/`에 보관,
     이후 수동 라벨링 대상
3. `model/scripts/subsample_frames.py` — frames_lying(2,754장)은 8fps 추출이라 인접
   프레임끼리 거의 동일해 그대로 전부 라벨링하기엔 너무 많고 중복도 심함. 직전에
   채택(keep)한 프레임과 그레이스케일 32x32 다운샘플 기준 평균 절대차(MAD)를 비교해
   충분히 달라진 프레임만 그리디하게 채택하는 방식으로 서브샘플링
   (`--threshold 7 --min-gap 4 --max-gap 16`, 8fps 기준 최소 0.5초·최대 2초 간격)
   → `model/data/own_capture/subsampled_frames/`
...
| 서있음/걷는 중 (제외) | 743장 |
| 누움(중복 포함) | 2,754장 |
| **서브샘플링 후 — 실제 라벨링 대상** | **282장** |

서브샘플링 결과를 그리드로 육안 확인한 결과, 빈 침대 → 이불을 당기는 동작 → 다양한
정도로 덮인 자세 → 다시 일어나는 동작까지 중복 없이 골고루 분포된 것을 확인했다.
목표(200~400장) 범위 중간 수준으로, 8개 영상 각각에서 16~65장씩 뽑혀 영상별 실제
동작 길이/변화량에 비례하는 자연스러운 분포를 보인다.

**라벨링 도구 선정 경위**: Roboflow(클라우드 업로드) → LabelImg(오프라인 시도, **실패**:
PyQt6/PyQt5 호환성 문제로 이미지 로딩이 계속 크래시, `save_file`의 `file_path` None
크래시까지 고쳤으나 근본 문제 해결 안 됨, `docs/LABELIMG_GUIDE.md`에 실패 기록으로 남김)
→ **[makesense.ai](https://www.makesense.ai/)로 최종 확정**. 브라우저 기반이지만 공식
저장소에 "이미지를 서버로 전송하지 않는다"고 명시된 완전 로컬 처리 도구라 오프라인
요구사항을 만족함. 사용법·라벨링 원칙·export 가져오기 절차는
**[docs/MAKESENSE_GUIDE.md](MAKESENSE_GUIDE.md)** 참고.

**최종 라벨링 결과 (2026-07-15)**: 서브샘플링된 282장 중 실제로 사람이 화면에 있는
**259장**에 대해 전체범위(full-extent) bbox 라벨링 완료 (class 0 = person, 이미지당
박스 1개). 나머지 23장(각 영상의 시작/끝 부분 — 빈 침대만 보이거나 사람이 화면 밖으로
나간 프레임)은 사람이 아예 없어 의도적으로 라벨링 대상에서 제외함. 최종적으로
`model/data/own_capture/labeled/{images,labels}/`에 이미지 259장 + 라벨 259개 (형식
검증 완료: 클래스 전부 0, 빈 라벨 0개, 이미지당 박스 1개).
...
**규모 판단**: 최종 라벨링된 259장(서브샘플링 282장 중 사람이 없는 23장 제외)은
목표치(500~1,000장)에는 못 미치지만, **단일 카메라 각도·단일 장소(침대)·8개 영상**에서만
나온 데이터라는 근본적 한계를 감안하면 무리하게 프레임 수를 늘리는 것보다 중복 없는
259장이 라벨링 관점에서 더 실용적이다. 다만 각도/거리/조명 다양성이 전혀 없다는 한계는
여전하다. 이 259장은 **occlusion이 연속적으로 진행되는 과정을 촘촘하게 담은 보조
데이터**로는 매우 유용하지만, SHOOTING_GUIDE.md가 원래 요구한 "각도/거리/조명 다양성"을
대체할 수는 없다. **권장**: 팀원 C·D에게 최소 2~3개 각도
(정면/측면/위쪽 등)에서 동일한 눕기→덮이기→일어나기 과정을 몇 개(3~5개 영상 정도)만 더
촬영해 달라고 요청할 것 — 전신이 아니라 각도 다양성만 보강하면 되므로 큰 부담은 아님.
...
### 5.1 실행 결과 (2026-07-15, 전체 소스 최종 분할)

COCO/CrowdHuman/WiderPerson(공개) + synthetic(합성) + own_capture(자체촬영, 259장 라벨링
완료) 전체를 대상으로 실행. OCHuman은 아직 미확보라 자동으로 건너뜀(0장).

| 소스 | 전체 | train | val | test |
|---|---|---|---|---|
| COCO | 66,808 | 53,446 | 6,681 | 6,681 |
| CrowdHuman | 19,370 | 15,496 | 1,937 | 1,937 |
| WiderPerson | 9,000 | 7,200 | 900 | 900 |
| OCHuman | 0 (미확보) | - | - | - |
| 합성(synthetic) | 95,175 | 76,140 | 9,518 | 9,517 |
| 자체촬영(own_capture) | 259 | 78 | 52 | 129 |
| **합계** | **190,612** | **152,360** | **19,088** | **19,164** |

모든 소스가 목표 비율(공개·합성 80/10/10, 자체촬영 30/20/50)과 정확히 일치함을
`model/data/splits/{train,val,test}.txt` 줄 수로 재검증 완료 (152,360 / 19,088 / 19,164).
자체촬영은 test 비중을 50%로 크게 잡은 설계대로, 259장 중 129장이 test에 배정됨 —
실사용 성능 검증 목적에 부합.
...
model/data/
  ...
  own_capture/            # 자체 촬영 (gitignore 처리)
    ...
    frames_lying/         # 누움(가려짐 진행 중), 중복 포함
    subsampled_frames/    # frames_lying 중복 제거 후 실제 라벨링 대상 (282장, 영상별 폴더)
    subsampled_frames_flat/  # 위와 동일한 282장, 라벨링 도구 업로드용 평탄 폴더 (makesense.ai에 사용)
...
- OCHuman 신청 폼 제출 및 bbox 성격(amodal 여부) 원 논문 재확인 — 확보 후
  `convert_to_yolo.py ochuman` → `split_dataset.py` 재실행 필요 (현재는 0장이라 자동 스킵됨)
- ~~자체 촬영 일정 협의 및 라벨링~~ → **완료** (연속 영상 8개 확보, 259장 makesense.ai로
  라벨링 완료, 2026-07-15 — 상세는 4.1절). 단, 단일 카메라 각도 한계로 팀원 C·D에게
  2~3개 각도 추가 촬영 요청 권장(4.1절 참고)은 여전히 유효
...
- ~~자체 촬영 데이터까지 모두 갖춰지면 `split_dataset.py`로 전체 train/val/test를 한 번에
  분할~~ → **완료** (총 190,612장 분할, train 152,360 / val 19,088 / test 19,164,
  2026-07-15 — 상세는 5.1절). PRD 8장 스모크테스트 결과 본 학습 규모 재검증은 Phase 3에서 진행
