...
--limit: 벤치마크/테스트용으로 앞에서부터 N장만 처리.
--max-boxes-per-image: CrowdHuman처럼 이미지 하나에 사람이 매우 많은 경우, occlusion을
    적용할 박스 수를 제한 (나머지 박스는 원본 그대로 두되 라벨에는 계속 포함됨).
"""
import argparse
import csv
import time
...
MAX_ATTEMPTS = 8  # 목표 비율에 수렴시키기 위한 최대 재시도 횟수 (이분탐색이라 8회면 1/256 정밀도)
...
def process_dataset(images_dir, labels_dir, out_dir, ratios, variants_per_image, seed=0,
                     limit=None, max_boxes_per_image=None, progress_every=200):
    ...
    with open(metadata_path, "a", newline="", encoding="utf-8") as f:
        ...
        for img_index, img_path in enumerate(image_paths):
            ...
            try:
                ...
                print(f"경고: {img_path.name} 로드 실패, 건너뜀 ({e})")
                continue
            boxes = read_yolo_labels(label_path, image.width, image.height)
            if not boxes:
                continue

            # CrowdHuman처럼 이미지 하나에 사람이 매우 많은 경우, 전원을 잔해로 덮으면
            # 비현실적이고 처리 시간도 급증하므로 occlusion을 적용할 박스 수를 제한
            occlude_target_boxes = boxes
            if max_boxes_per_image and len(boxes) > max_boxes_per_image:
                idx = rng.choice(len(boxes), size=max_boxes_per_image, replace=False)
                occlude_target_boxes = [boxes[i] for i in sorted(idx)]

            for variant in range(variants_per_image):
                # 이미지 인덱스+variant를 함께 순환시켜, variants_per_image=1이어도
                # 이미지 전체 기준으로는 30/50/70%가 고르게 섞이도록 함
                target_ratio = ratios[(img_index + variant) % len(ratios)]
                occluded = image.copy()
                variant_meta = []
                occluded_box_set = set(id(b) for b in occlude_target_boxes)
                for i, box in enumerate(boxes):
                    if id(box) in occluded_box_set:
                ...
                # occlusion 대상에서 제외된 박스도 라벨에는 그대로 포함)
                lines = []
            ...
            n_done += 1
            if n_done % progress_every == 0 or n_done == len(image_paths):
                elapsed = time.time() - start
                rate = n_done / elapsed if elapsed > 0 else 0
                eta = (len(image_paths) - n_done) / rate if rate > 0 else float("inf")
                print(f"진행: {n_done}/{len(image_paths)} - {rate:.1f}장/초, "
                      f"경과 {elapsed:.0f}초, 예상 잔여 {eta:.0f}초")
...
def main():
    ...
    parser.add_argument("--limit", type=int, default=None, help="테스트/벤치마크용 상한 (앞에서부터 N장만)")
    parser.add_argument("--max-boxes-per-image", type=int, default=None,
                         help="이미지당 occlusion을 적용할 최대 박스 수 (CrowdHuman처럼 밀집된 "
                              "이미지에서 전원을 덮어 비현실적이거나 처리시간이 급증하는 것을 방지)")
    args = parser.parse_args()

    process_dataset(args.images, args.labels, args.out, args.ratios, args.variants_per_image,
                     args.seed, limit=args.limit, max_boxes_per_image=args.max_boxes_per_image)
